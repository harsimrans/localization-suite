	...
	for g in GRID_SIZE:
		...
		for n in NOISE:
			...
			for i in range(44):
				...
				for j in range(44):
					if i != j:
						dist = (loc[0][index] - loc[0][j])**2 + (loc[1][index] - loc[1][j])**2 
						if dist <= (RADIUS ** 2):
				...
				print "location of the transmitter at", i, " : ", location_transmitter, " actual: ", loc[0][i], loc[1][i], " error: ", get_distance(location_transmitter[0], location_transmitter[1], loc[0][i], loc[1][i]), " receivers used: ", len(receivers)
